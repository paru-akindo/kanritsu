...
herb_cycles   = 40           # 1仙草あたりの補助周天数
buff_options  = {"30%": 0.30, "20%": 0.20, "10%": 0.10, "3%": 0.03}

# ── シミュレーション関数 ──
def simulate_time(remaining, base_speed, buff, init_herb_count=0):
    ...
    while True:
        ...
        item_herb_pts = init_herb_count * herb_cycles * base_speed

        if manual_pts + time_herb_pts + item_herb_pts >= remaining:
            # 最小時間を探す
            while t > 0:
                t2  = t - 1
                m2  = (t2 / cycle_time) * base_speed * (1 + buff)
                th2 = (t2 // herb_interval) * herb_cycles * base_speed
                if m2 + th2 + item_herb_pts < remaining:
                    break
                t = t2
            return t, manual_pts, time_herb_pts, item_herb_pts
        t += 1
...
if st.button("シミュレーション開始"):
    ...
    for label, buff in buff_options.items():
        t0, m0, th0, ih0 = simulate_time(remaining, base_speed, buff, init_herb_count=0)
        rows_unused.append({
            "バフ":             label,
    ...
    if items > 0:
        ...
        rows_used = []
        for label, buff in buff_options.items():
            t1, m1, th1, ih1 = simulate_time(
                remaining, base_speed, buff, init_herb_count=used_items * 3
            )
            rows_used.append({
                "バフ":             label,
