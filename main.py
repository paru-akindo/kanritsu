...
def simulate_time(remaining, base_speed, buff, init_herbs=0):
    ...
    while True:
        ...
        total_herb = (init_herbs + (t//herb_interval)) * herb_cycles * base_speed
        if manual_pts + total_herb >= remaining:
            while t > 0:
                t2 = t - 1
                if ((t2/cycle_time)*base_speed*(1+buff)
                   + (init_herbs + (t2//herb_interval))*herb_cycles*base_speed
                   ) < remaining:
                    break
                t = t2
            return t, manual_pts, init_herbs, total_herb - init_herbs, total_herb
        t += 1
...
if st.button("シミュレーション開始"):
    ...
    for label, buff in buff_options.items():
        t0, m0, ih0, th0, th_total0 = simulate_time(remaining, base_speed, buff, init_herbs=0)
        rows_unused.append({
            "バフ":             label,
    ...
    if items > 0:
        ...
        rows_used = []
        for label, buff in buff_options.items():
            t1, m1, ih1, th1, th_total1 = simulate_time(
                remaining, base_speed, buff, init_herbs=used_items*3
            )
            rows_used.append({
                "バフ":             label,
