...
if st.button("シミュレーション開始"):
    ...
    if remaining <= 0:
        ...
        for label, buff in buff_options.items():
            ...
            h_0, m_0   = t0 // 3600, (t0 % 3600) // 60

            row = {
                "バフ":                label,
                "未使用 到達時刻":      finish0.strftime("%Y-%m-%d %H:%M"),
                "未使用 所要時間":      f"{h_0}h {m_0}m",
                "未使用 手動修練(万)": f"{int(m0)//10000}",
                "未使用 仙草修練(万)": f"{int(h0)//10000}",
            }
            ...
            if items > 0:
                ...
                init_herb_cnt  = used_items * 3

                t1, m1, h1 = simulate_time(remaining, base_speed, buff, init_herbs=init_herb_cnt)
                finish1    = now_jst + timedelta(seconds=t1)
                h_1, m_1   = t1 // 3600, (t1 % 3600) // 60

                row[f"{used_items}個使用 到達時刻"]      = finish1.strftime("%Y-%m-%d %H:%M")
                row[f"{used_items}個使用 所要時間"]      = f"{h_1}h {m_1}m"
                row[f"{used_items}個使用 手動修練(万)"] = f"{int(m1)//10000}"
                row[f"{used_items}個使用 仙草修練(万)"] = f"{int(h1)//10000}"
